...
//  SDL_SCANCODE_* codes, e.g. ImGui::IsKeyPressed(SDL_SCANCODE_SPACE).
// Missing features:
...
//  compiling SDL with SDL_DISABLE_WINDOWS_IME. [ ] Platform: Gamepad support
//  (need to use SDL_GameController API to fill the io.NavInputs[] value when
//  ImGuiConfigFlags_NavEnableGamepad is set).
...
static void ImGui_ImplSDL2_UpdateMousePosAndButtons() {
  ...
  if (g_Window == focused_window) {
    /* TODO for newer version in is right, but for 2.0.4 sdl it is wrong*/
...
void ImGui_ImplSDL2_NewFrame(SDL_Window *window) {
  ImGuiIO &io = ImGui::GetIO();
  IM_ASSERT(io.Fonts->IsBuilt()); // Font atlas needs to be built, call renderer
                                  // _NewFrame() function e.g.
